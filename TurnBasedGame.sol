...
contract TurnBasedGame{
    ...
    Game game;
       ...
       // Mapping of key to games
    mapping (uint256 => Game) games;
    
    event NewGame(address player1, address player2, uint256 score);
    
    constructor(uint256 uid, address _player2, uint256 _baseScore) {
        newGame(uid, _player2, _baseScore);
    }
    
    
    modifier checkCalledPlayer(uint256 uid){
    ...
    function updateScore(uint256 uid, uint256 _score1, uint256 _score2) public checkCalledPlayer(uid) {
        games[uid].score1 = _score1;
        games[uid].score2 = _score2;
    }
    
    function tieGame(uint256 uid) public checkCalledPlayer(uid) { //game - tied
        games[uid].state = gameState.TIED;
    }
    
    
    function wonGame(uint256 uid) public checkCalledPlayer(uid) { // game - over
        games[uid].state = gameState.WON;
    }
    
    function getInfo(uint256 uid) public view returns(address, address, uint256, uint256, gameState){
        return(games[uid].player1, games[uid].player2, games[uid].score1, games[uid].score2, games[uid].gameState);
    }
